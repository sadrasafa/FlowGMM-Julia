function train(;in_dim=768, hidden_dim=512, num_coupling_layers=7, num_hidden_layers=1, k=256, lr=1e-4, steps=5001, print_freq=1000, batch_size=80, random_seed=-1, dataset="yahoo_answers", latent_mean_random=true)
    ...
    for step in 1:steps
        batch_idx_ul = Distributions.sample(1:num_unlabeled, batch_size, replace=false)
        batch_x_ul, batch_y_ul = unlabeled_data[:, batch_idx_ul], unlabeled_labels[batch_idx_ul]
        ...
        batch_x = hcat(batch_x_ul, batch_x_l)
        batch_y = vcat(batch_y_ul, batch_y_l)
